  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResponseMessage(
          errData.error || "Erro ao enviar a mensagem. Tente novamente."
        );
          ...
          <aside className="flex flex-col justify-center space-y-8">
            ...
            <div className="bg-gradient-to-br from-[#23243a]/60 to-[#1a1b2e]/60 backdrop-blur-sm p-6 rounded-2xl border border-cyan-500/20">
              ...
              <p className="text-gray-300 text-sm leading-relaxed">
                Normalmente respondo emails em até{" "}
                <span className="text-cyan-400 font-semibold">24 horas</span> e
                mensagens via WhatsApp em algumas horas durante horário
                comercial.
              </p>
            </div>
          </aside>
...
function ContactCard({
  ...
  const content = (
    <div className="group bg-gradient-to-br from-[#23243a]/60 to-[#1a1b2e]/60 backdrop-blur-sm p-6 rounded-2xl border border-transparent hover:border-cyan-500/30 transition-all duration-300 hover:scale-105">
      <div className="flex items-start gap-4">
        <div
          className={`p-3 bg-gradient-to-r ${color} rounded-xl text-white shadow-lg`}
        >
          {icon}
        </div>
  ...
  return href ? (
    <a
      href={href}
      className="block focus:outline-none focus:ring-2 focus:ring-cyan-400 rounded-2xl"
    >
      {content}
    </a>
  ) : (
